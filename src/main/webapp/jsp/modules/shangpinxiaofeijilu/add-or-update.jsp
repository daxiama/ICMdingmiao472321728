...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑商品消费记录</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>商品消费记录管理</span></li>
								<li class="breadcrumb-item active"><span>编辑商品消费记录</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">商品消费记录信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>订单号</label>
										<select  class="form-control"  name="dingdanhao" id="dingdanhaoSelect" onchange="dingdanhaoSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>商品名称</label>
									...
									<div class="form-group col-md-6">
										<label>价格</label>
										<input id="jiage" name="jiage" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>数量</label>
										<input id="shuliang" name="shuliang" class="form-control"  value="" readonly>
									</div>
									<div class="form-group col-md-6">
										<label>金额</label>
	...
	<script>
		...
		var tableName = "shangpinxiaofeijilu";
		...
		function dingdanhaoSelect2(){
			http("option/shangpindingdan/dingdanhao","GET",{
				tableName: "shangpindingdan",
				columnName: "dingdanhao"
			},(res)=>{
				if(res.code == 0){
					var options = res.data;
					var option = document.createElement("option");
					$("#dingdanhaoSelect").append(option)
					for(var i=0;i<options.length;i++){
						var dingdanhaoOption = document.createElement("option");
						dingdanhaoOption.setAttribute("value",options[i]);
						dingdanhaoOption.setAttribute("class","dingdanhaoOption");
						if(ruleForm.dingdanhao == options[i]){
							dingdanhaoOption.setAttribute("selected","selected");
						}
						dingdanhaoOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.dingdanhao != null && ruleForm.dingdanhao != '' && options[i] == ruleForm.dingdanhao){
							dingdanhaoOption.setAttribute("selected","selected");
						}
						$("#dingdanhaoSelect").append(dingdanhaoOption);
					}
				}
			});
		}		
		function dingdanhaoSelect2Change(){
			var options = document.getElementById("dingdanhaoSelect").options;
			var index = document.getElementById("dingdanhaoSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/shangpindingdan/dingdanhao","GET",{
				tableName: "shangpindingdan",
				columnName: "dingdanhao",
				columnValue: colVal
			},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data.jiage != null){
						$("#jiage").val(res.data.jiage);
					}
					if(res.data != null && res.data.shuliang != null){
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "shangpinxiaofeijilu/info/" + id,
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
				...
				if(crossrefid && crossuserid) {
					...
					http("shangpinxiaofeijilu/page","GET",{
						page:1,
						limit:10,
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("shangpinxiaofeijilu/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("shangpinxiaofeijilu/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("shangpinxiaofeijilu"+colName,"GET",{
