...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑上机信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>上机信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑上机信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">上机信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>上机时间</label>
										<div id="shangjishijianDate" class="input-append date form_datetime">
											<input id="shangjishijian-input" name="shangjishijian" size="20" type="text" readonly>
									...
									<div class="form-group col-md-6">
										...
										<select id="zhuangtaiSelect" name="zhuangtai" class="form-control">
												<option value=""></option>
												<option class="zhuangtaiOption" value="已下机">
													已下机
												</option>
												<option class="zhuangtaiOption" value="上机">
													上机
												</option>
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>机号</label>
										<select  class="form-control"  name="jihao" id="jihaoSelect" onchange="jihaoSelect2Change()">
										</select>
									</div>
									<div class="form-group col-md-6">
										<label>单价</label>
	...
	<script>
		...
		function jihaoSelect2(){
			http("option/diannaoxinxi/jihao","GET",{
				...
				if(res.code == 0){
					...
					$("#jihaoSelect").append(option)
					for(var i=0;i<options.length;i++){
						var jihaoOption = document.createElement("option");
						jihaoOption.setAttribute("value",options[i]);
						jihaoOption.setAttribute("class","jihaoOption");
						if(ruleForm.jihao == options[i]){
							jihaoOption.setAttribute("selected","selected");
						}
						jihaoOption.innerHTML = options[i];
						//修改 回显
						if(ruleForm.jihao != null && ruleForm.jihao != '' && options[i] == ruleForm.jihao){
							jihaoOption.setAttribute("selected","selected");
						}
						$("#jihaoSelect").append(jihaoOption);
					}
				}
			});
		}		
		function jihaoSelect2Change(){
			var options = document.getElementById("jihaoSelect").options;
			var index = document.getElementById("jihaoSelect").selectedIndex;
			var colVal = options[index].value;
			http("follow/diannaoxinxi/jihao","GET",{
				tableName: "diannaoxinxi",
				columnName: "jihao",
				columnValue: colVal
			},(res)=>{
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "shangjixinxi/info/" + id,
				...
				if(crossrefid && crossuserid) {
					...
					http("shangjixinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("shangjixinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("shangjixinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
				...
				for(var zhuangtaiCount = 0; zhuangtaiCount < zhuangtaiOptions.length;zhuangtaiCount++){
					if(zhuangtaiOptions[zhuangtaiCount].getAttribute('value') == "上机"){
		...
		function dateTimePick(){
			...
			$("#shangjishijian-input").val(getCurDateTime());
			$("#shangjishijianDate").datetimepicker({ 
				autoclose: true,
		...
		function dataBind(){
			...
			$("#shangjishijian-input").val(ruleForm.shangjishijian);
			var zhuangtaiOptions = document.getElementsByClassName("zhuangtaiOption");
			for(var zhuangtaiCount = 0; zhuangtaiCount < zhuangtaiOptions.length;zhuangtaiCount++){
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.shangjishijian != null && res.data.shangjishijian != ''){
						...
						$("#shangjishijian-input").val(res.data.shangjishijian);
						$('#shangjishijianFileName').val(res.data.shangjishijian)
					}
